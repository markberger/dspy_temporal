...
`docker compose up` binds fixed host ports (`7233`/`8233` Temporal, `6006`/`4317`
Phoenix). Only **one** worktree may run the live stack at a time. To run two,
set `COMPOSE_PROJECT_NAME` and remap the published ports in an override. Likewise,
if you point a bare `examples/worker.py` at a shared `localhost:7233`, give each
worktree a **distinct `task_queue`** so workers don't split each other's tasks.
