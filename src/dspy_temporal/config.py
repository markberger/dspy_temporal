...
class RunConfig:
    ...
    - ``"coarse"`` (default): the whole ``dspy.Module`` runs in one activity;
      durability is job-level (a crash re-runs the whole program).
    - ``"fine"``: each LM call and each tool call is its own activity,
